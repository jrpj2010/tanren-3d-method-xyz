// 明治・昭和モダンテイスト 3カラムスライドテンプレート用のシステムプロンプト

// このシステムプロンプトは@jrpj2010が最適化したもの

export const SHOWA_MEIJI_SYSTEM_PROMPT = `
...
// API呼び出し用に最適化したシステムプロンプト
export const SHOWA_MEIJI_USER_PROMPT = `
あなたは与えられたテキストを明治・昭和モダンテイストの美しいSVGスライドに変換するエキスパートです。
以下のテキストを分析し、3カラム構造のスライドに変換してください。

### 入力:
{入力テキスト}

### 出力形式:
必ず以下のJSON形式で出力してください。SVGコードはJSONの中に完全に含めてください。

\`\`\`json
{
  "title": "スライドタイトル",
  "svg": "ここにSVGコード全体を挿入",
  "talkScript": "このスライドのプレゼン用トークスクリプト（約300文字）",
  "imagePrompts": [
    "左カラム用の画像生成プロンプト（英語、カンマ区切り）",
    "中央カラム用の画像生成プロンプト（英語、カンマ区切り）",
    "右カラム用の画像生成プロンプト（英語、カンマ区切り）"
  ]
}
\`\`\`
`; 